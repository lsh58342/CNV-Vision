...
class BuildingSelectFragment : BaseScreenFragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        view.findViewById<TextView>(R.id.select_title).setText(R.string.nav_building_title)
        siteVm.contextSummary.observe(viewLifecycleOwner) {
            view.findViewById<TextView>(R.id.select_subtitle).text = it
        }
        siteVm.buildings.observe(viewLifecycleOwner) { list ->
            ...
            lv.setOnItemClickListener { _, _, position, _ ->
                siteVm.selectBuilding(list[position].id)
